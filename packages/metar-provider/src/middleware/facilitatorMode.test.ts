...
import {
  ...
  MiddlewareOptions,
  verifyPaymentViaFacilitator,
} from "./createX402Middleware.js";
...
test("verifyPaymentViaFacilitator - returns true when facilitator verifies payment", async () => {
  ...
  const result = await verifyPaymentViaFacilitator(facilitatorUrl, "test-tx-sig", "test:v1", 0.01);
...
test("verifyPaymentViaFacilitator - handles facilitator URL with trailing slash", async () => {
  ...
  await verifyPaymentViaFacilitator(facilitatorUrl, "test-tx-sig", "test:v1", 0.01);
...
test("createX402Middleware - uses direct verification when facilitatorMode is disabled", async () => {
  ...
  assert.ok(
    !facilitatorCalled,
    "Facilitator should not be called when facilitatorMode is disabled"
  );
  // Payment verification should fail (no real transaction), so we get 402
  assert.strictEqual(responseStatus, 402);
...
test("createX402Middleware - uses direct verification when facilitatorMode is undefined", async () => {
  ...
  assert.ok(
    !facilitatorCalled,
    "Facilitator should not be called when facilitatorMode is undefined"
  );
  // Payment verification should fail (no real transaction), so we get 402
  assert.strictEqual(responseStatus, 402);
