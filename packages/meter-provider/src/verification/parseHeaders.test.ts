...
test("parseAuthorizationHeader - parses valid signature header", () => {
  const header = 'Signature keyId="agent_12345", alg="ed25519", headers="(request-target) date x-meter-nonce x-meter-tx", signature="dGVzdA=="';
  ...
  assert.deepStrictEqual(result!.headers, ["(request-target)", "date", "x-meter-nonce", "x-meter-tx"]);
...
test("parseAuthorizationHeader - handles headers with extra spaces", () => {
  const header = 'Signature keyId="agent_12345" , alg="ed25519" , headers="(request-target) date" , signature="test"';
...
test("parseAuthorizationHeader - handles quoted values correctly", () => {
  const header = 'Signature keyId="agent_12345", alg="ed25519", headers="(request-target) date", signature="dGVzdA=="';
