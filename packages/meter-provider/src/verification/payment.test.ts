...
class MockConnection extends Connection {
  ...
  async getTransaction(signature: string, options?: any): Promise<ParsedTransactionWithMeta | null> {
    return this.mockTx;
  }
}
...
test("verifyPayment - verifies valid payment without memo", async () => {
  ...
  (mockTx.transaction.message.instructions[0] as any).parsed.info.destination = recipientATA.toString();
  ...
  const result = await verifyPayment(connection, "test-tx-sig", usdcMint, payTo, expectedAmount, headers);
