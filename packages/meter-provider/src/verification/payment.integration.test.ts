...
import { PaymentHeaders, PaymentMemo } from "@meter/shared-types";
import { createConnection, getUSDCMint, MEMO_PROGRAM_ID } from "@meter/shared-config";
import { buildUSDCTransfer } from "@meter/meter-client";
import { sendPayment } from "@meter/meter-client";
...
test("verifyPayment - integration test with real devnet transaction", { skip: skipIntegration }, async () => {
...
test("verifyPayment - integration test with mismatched memo", { skip: skipIntegration }, async () => {
  const connection = createConnection("devnet");
  const usdcMint = getUSDCMint("devnet");
  ...
  assert.strictEqual(isValid, false, "Payment verification should fail with mismatched memo nonce");
});
