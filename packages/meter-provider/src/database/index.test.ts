...
test("initializeDatabase - creates table and indexes", async () => {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='usage_records'", (err, row) => {
        if (err) reject(err);
        else {
    ...
    await new Promise<void>((resolve, reject) => {
      db.all("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='usage_records'", (err, rows) => {
        if (err) reject(err);
        else {
...
test("logUsage - inserts usage record successfully", async () => {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      db.get(
        "SELECT * FROM usage_records WHERE tx_sig = ?",
        [headers.txSig],
        (err, row: any) => {
          if (err) reject(err);
          else {
