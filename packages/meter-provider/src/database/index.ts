 ...
 * ```typescript
 * import { Database } from "sqlite3";
 * import { initializeDatabase } from "@meter/meter-provider";
 * 
 ...
 * ```typescript
 * import { Database } from "sqlite3";
 * import { logUsage } from "@meter/meter-provider";
 * import { PaymentHeaders } from "@meter/shared-types";
 * 
 * const db = new Database("usage.db");
 ...
 * ```typescript
 * import { Database } from "sqlite3";
 * import { isTransactionUsed } from "@meter/meter-provider";
 * 
 * const db = new Database("usage.db");
 ...
 * ```
 */
export function isTransactionUsed(
  db: Database,
  txSig: string
): Promise<boolean> {
  return new Promise((resolve, reject) => {
    db.get(
      `SELECT 1 FROM usage_records WHERE tx_sig = ? LIMIT 1`,
      [txSig],
      (err, row) => {
        if (err) {
          reject(err);
