 ...
 * import { createPaidFetch, createNodeWallet } from "@meter/meter-client";
  ...
  return async function paidFetch(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        // Note: Using UUID v4 for now. UUID v7 (time-based) should be used when uuid@9+ is available.
        // UUID v7 provides better sortability and includes timestamp information.
        const nonce = uuidv4();
