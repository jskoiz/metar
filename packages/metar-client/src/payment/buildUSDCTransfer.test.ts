...
test(
  "buildUSDCTransfer - creates transaction with transfer instruction",
  { skip: skipTests },
  async () => {
...
test(
  "buildUSDCTransfer - includes memo instruction when memo provided",
  { skip: skipTests },
  async () => {
    const connection = createConnection("devnet");
    const payer = Keypair.generate();
    ...
    const memoInstruction = transaction.instructions.find(ix =>
