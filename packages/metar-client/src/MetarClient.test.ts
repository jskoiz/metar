/**
 * Unit tests for MetarClient.
 *
 * These tests verify that MetarClient properly initializes, validates configuration,
 * and delegates requests to the underlying paid fetch wrapper.
 */

import { Connection, Keypair, Transaction } from "@solana/web3.js";
import { MetarClient, MetarClientConfig } from "./MetarClient.js";
...
async function runTests() {
  ...
  const validConfig: MetarClientConfig = {
  ...
  if (
    await test("MetarClient - creates instance with valid configuration", () => {
      try {
        const client = new MetarClient(validConfig);
        return client instanceof MetarClient;
      } catch (error) {
        return false;
      }
    })
  ...
  if (
    await test("MetarClient - creates instance without chain", () => {
      const configWithoutChain = { ...validConfig };
      delete configWithoutChain.chain;
      try {
        const client = new MetarClient(configWithoutChain);
        return client instanceof MetarClient;
      } catch (error) {
        return false;
      }
  ...
  if (
    await test("MetarClient - throws error if providerBaseURL is missing", () => {
      const invalidConfig = { ...validConfig, providerBaseURL: "" };
      try {
        new MetarClient(invalidConfig);
        return false;
      } catch (error) {
        return (
          error instanceof Error && error.message.includes("Invalid MetarClient configuration")
        );
      }
    })
  ) {
  ...
  if (
    await test("MetarClient - throws error if agentKeyId is missing", () => {
  ...
  if (
    await test("MetarClient - throws error if agentPrivateKey is empty", () => {
  ...
  if (
    await test("MetarClient - throws error if wallet is missing", () => {
  ...
  if (
    await test("MetarClient - throws error if connection is missing", () => {
  ...
  if (
    await test("MetarClient - request method exists", () => {
      try {
        const client = new MetarClient(validConfig);
